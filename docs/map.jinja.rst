...
We will see a quick setup to configure the ``btc`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``btc`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``btc`` formulas under ``/srv/salt/btc/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/btc/parameters/dns:domain/
    ln -s dns:domain /srv/salt/btc/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/btc/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/btc/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``btc`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``btc`` formulas under ``/srv/salt/btc/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/btc/parameters/roles
...
We create a configuration for the role ``btc/server`` in ``/srv/salt/btc/parameters/roles/btc/server.yaml``:
...
We create another configuration for the role ``btc/client`` in ``/srv/salt/btc/parameters/roles/btc/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles btc/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles btc/client
...
.. code-block:: console
    ...
    mv /srv/salt/btc/parameters/dns:domain/  '/srv/salt/btc/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-btc-as-json:
      test.nop:
        - name: {{ btc | json }}
...
Here is an example based on `template-formula/btc/config/file.sls`_:
...
This ``sls`` file expose a ``btc`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ btc['winner'] }}
...
.. _template-formula/btc/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/btc/config/file.sls
